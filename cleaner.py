...
csv_file_name = 'portscan_v2.csv'

# Open the JSON file and the CSV file for writing
with open('portscan_.json', 'r') as json_file, open(csv_file_name, 'w', newline='') as csv_file:
    fieldnames = ['avg_ipt', 'bytes_in', 'bytes_out', 'dest_ip', 'dest_port', 'entropy', 'proto',
                  'src_ip', 'src_port', 'num_pkts_out', 'num_pkts_in', 'total_entropy', 'label', 'duration']
    ...
    for line in json_file:
        ...
        row = {
            ...
            'dest_ip': event.get('da', ''),  # Convert destination IP
            'dest_port': event.get('dp', 0),
            'entropy': event.get('entropy', 0.0),
            'proto': int(event.get('pr', 0)),
            'src_ip': event.get('sa', ''),  # Convert source IP
            'src_port': event.get('sp', 0),
            'num_pkts_out': int(event.get('num_pkts_out', 0)),
            'num_pkts_in': int(event.get('num_pkts_in', 0)),
